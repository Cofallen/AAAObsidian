...
> [!summary]+ 📅 本月各周计划
> ```dataviewjs
> const currentMonth = dv.current().file.name;
> const weeks = dv.pages('"MyLife/Weekly"').sort(p => p.file.name);
>
> let found = false;
>
> for (const page of weeks) {
>
>     const start = window.moment(page.file.name, "GGGG-[W]WW").startOf("isoWeek");
>     const end   = window.moment(page.file.name, "GGGG-[W]WW").endOf("isoWeek");
>
...
>         const section = t.section?.subpath ?? "";
>         return section.includes("每周计划");
>     });
>
>     if (tasks.length === 0)
>         continue;
